...
class ExpectColumnValuesToBeGreaterThanOneHundred(Expectation): # 自定义期望继承Expectation类
    ...
    # TODO: 尝试通过result_format = (runtime_configuration or {}).get("result_format", "BASIC")来适配result_format参数
    # TODO: 尝试使用使用 metric-based 实现（比如 ColumnValuesMetricProvider）
    
# 1. 读取测试数据
test_df = pd.read_csv(os.path.join(current_dir, "test_data_01.csv"))
...
# 9. 输出校验结果
with open(os.path.join(current_dir, "test_result_03.json"), "w", encoding="utf-8") as f:
    f.write(str(results))
